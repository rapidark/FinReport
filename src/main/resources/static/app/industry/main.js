 ...
 */
define(['jquery', 'domReady', 'pinyin', 'bloodhound', 'industry/companyselect', 'industry/industryCompanyReport', 'typeahead', 'json', 'textfill', 'select', 'bootstrap'], function ($, domReady, Pinyin, Bloodhound, companyselect, industryCompanyReport) {
    ...
    function initCompaniesSelection(companies, industryName) {
        ...
        $.each(companies, function(i) {
            ...
            checkedCompaniesName.push(this.name);
        });
    ...
    function renderCompanyPanel(container, selectCompanies, totalCount) {
        ...
        for (var i = 0; i < checkedCompanies.length; i++) {
            if(selectCompanies.map(function(e) {return e.code}).indexOf(checkedCompanies[i]) == -1) {
                ...
                checkedCompaniesName.splice(i, 1);
                i--
            }
        }
            ...
            company.on('change', function () {
                if(this.checked) {
                    if(checkedCompanies.length < 10) {
                        ...
                        checkedCompaniesName.push(item.name);
                    } else {
                        this.checked = false;
                        $.alert('最多只能选择10家公司');
                    }
                    ...
                    if(index > -1) {
                        ...
                        checkedCompaniesName.splice(index, 1);
                    }
                }
            });
    ...
    domReady(function () {
        ...
        $('#confirmIndustryCompanies').on('click', function () {
            var container = $('#industryCompanyDataContainer');
            if(checkedCompanies.length > 0) {
                industryCompanyReport.render(container, checkedCompanies, checkedCompaniesName);
            } else {
                container.empty();
            }
        });
